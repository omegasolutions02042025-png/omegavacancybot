...
6. Всегда заполняй все поля, даже если их значение `null`.
...
ПЕРСОНАЛЬНЫЕ ДАННЫЕ
Если в резюме отсутствуют обязательные сведения, добавь их как отдельные must_have:
...
Если часть must_have = "Нет (требуется уточнение)" → Частично подходит (нужны уточнения).

Если большинство must_have = "Нет (точно нет)" → Не подходит.

📏 ОБЯЗАТЕЛЬНАЯ СТРУКТУРА JSON
Если ты не можешь определить значение поля — всё равно включи его, но поставь null.
...
Если резюме содержит даты в виде 05/2019 или 2019-05, преобразуй в 05.2019.

Все города, страны и имена переводи на русский.

Если встречаются английские имена (Ivan Ivanov) — преобразуй в Иван Иванов.

При указании возраста округляй до целого числа лет.
