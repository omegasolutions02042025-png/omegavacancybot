...
def sverka_vac_and_resume_json(vacancy_text: str, resume_text: str):
    ...
    prompt = f"""Твоя роль: Ты — продвинутый ассистент IT-рекрутера.

Твоя задача: Тебе будут предоставлены два блока текста: [ВАКАНСИЯ] и [РЕЗЮМЕ]. Твоя цель — внимательно проанализировать резюме кандидата на соответствие требованиям вакансии и структурировать абсолютно всю извлеченную и проанализированную информацию в единый JSON-объект. Не добавляй никакой информации от себя, кроме прямого анализа соответствия. Следуй строго указанной ниже JSON-структуре.

Выходной формат: JSON.

Инструкции по заполнению JSON:

vacancy: Перенеси ключевую информацию из текста [ВАКАНСИЯ] в соответствующие поля.

candidate: Извлеки всю информацию из текста [РЕЗЮМЕ] и заполни соответствующие поля.

candidate.full_name: Извлеки полное имя из текста [РЕЗЮМЕ] в формате "Имя Фамилия Отчество".Если указаны имена на английском, то переведи их на русский.
candidate.location.city: Извлеки город из текста [РЕЗЮМЕ].
candidate.location.country: Извлеки страну из текста [РЕЗЮМЕ].
...
{{
  ...
  "summary": {{
    ...
    model = genai.GenerativeModel('gemini-2.5-flash')
    # Температура 0.1 отлично подходит для задач извлечения и структурирования
    generation_config = genai.types.GenerationConfig(temperature=0.1)
    
    try:
        response = model.generate_content(prompt, generation_config=generation_config)
        
        # Модель должна вернуть только JSON, но если она добавит пояснения,
        # этот шаг может потребовать дополнительной очистки строки.
        # Для начала просто возвращаем текст.
        return response.text.strip() 
...
async def generate_mail_for_candidate_finalist(json_data: dict):
  ...
  6️⃣ Подпись:
  «С уважением,  
  [Имя / Ник менеджера из {vacancy['manager_telegram_nickname']} или указанный рекрутером]»
  ...
  Здравствуйте, Дмитрий Ефименко!
  Благодарим вас за резюме и интерес к нашей вакансии.

  Мы ищем Go разработчика для работы над высоконагруженными распределенными системами и проектирования микросервисной архитектуры. В работе используем Go, Kafka, gRPC, PostgreSQL, Redis, GitLab CI/CD, Docker, Kubernetes, Debezium, Prometheus, Grafana, Jaeger, Git, Protobuf, а также принципы SOLID, DDD, REST. Формат работы — удалённый. Предлагаем ежемесячную выплату: Штат/Контракт — 204 000 RUB, ИП — 244 000 RUB, Самозанятый — 275 000 RUB. Проект рассчитан на 3+ месяца со стартом ASAP, с отсрочкой платежа 35 рабочих дней после подписания акта.

  Нас особенно заинтересовали ваш опыт коммерческой разработки на Golang от 4-5 лет, глубокое понимание принципов многопоточности и параллелизма в Go, а также опыт проектирования и разработки микросервисной архитектуры и работы с PostgreSQL.
