...
6.📎 Анализ имени файла:
  ...
  1. Проанализируй содержимое переменной file_name: {file_name}.
  2. Раздели имя файла по символам `_`, `-`, `.`, пробелам.
  3. Проверь каждое слово:
    — если слово содержит кириллицу или латиницу и начинается с заглавной буквы, это часть ФИО;
    — первые два таких слова считай фамилией и именем.
    Пример:
    • Ivan_Ivanov_BE10451.docx → Имя = Иван, Фамилия = Иванов.
    • Kurs_Aleksandr_Resume.docx → Имя = Александр, Фамилия = Курс.
    • Alexander_Kurs_Resume.pdf → Имя = Александр, Фамилия = Курс.
  4. Если слово содержит числа и валютные обозначения (`USD`, `руб`, `у.е`, `$`) — это зарплатные ожидания. 
    Пример:
    • Ivan_Ivanov_3500USD.docx → salary_expectations = "3500 USD".
  5. Если найдено только одно имя или фамилия, добавь в must_have:
    {{
  ...
  6. Если не найдено ни одного слова с заглавной буквы — оставь статус `"Нет (требуется уточнение)"`.

  ---

## 🧮 ЛОГИКА СОВПАДЕНИЙ

- Если ≥70% обязательных требований совпали → оставшиеся требования, по которым нет подтверждения,  
  автоматически помечай `"Нет (требуется уточнение)"`.
- Если совпадений <70% → все неподтверждённые требования помечай `"Нет (точно нет)"`.

---

## 📋 ОСОБЫЕ ПРАВИЛА

- Отсутствие ФИО, локации или зарплаты не влияет на итоговый вердикт.  
  Эти пункты отмечаются как `"Нет (требуется уточнение)"` только для полноты.
...
- Все `must_have = Да` → `"Полностью подходит"`
- Если часть `must_have = Нет (требуется уточнение)` (и нет `"Нет (точно нет)"`) → `"Частично подходит (нужны уточнения)"`
- Если большинство `"Нет (точно нет)"` → `"Не подходит"`
