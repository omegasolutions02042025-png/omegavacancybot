...
def sverka_vac_and_resume_json(vacancy_text: str, resume_text: str):
    ...
    prompt = f"""
Ты — аналитик по найму.  
Твоя задача — ПРОАНАЛИЗИРОВАТЬ предоставленный текст, где описана вакансия и один кандидат.  
Ты должен извлечь всю информацию и структурировать её строго по шаблону ниже, возвращая результат **ТОЛЬКО в формате JSON** (без комментариев и пояснений).  

─────────────────────────────
ШАГ 1. ВАКАНСИЯ — извлеки:
• название позиции и ID (🆔);
• грейд;
• локация / гражданство / часовой пояс;
• формат работы (remote / office / hybrid);
• обязательные требования (must-have);
• дополнительные требования (nice-to-have);
• стек технологий;
• особые условия (NDA, SIM-карта, ставка, дедлайны и др.);
• ник Telegram менеджера (если есть).
...
─────────────────────────────
ШАГ 2. КАНДИДАТ — извлеки:
• ФИО/ФИ (с отчестом, если есть);
• дату рождения (формат «ДД.ММ.ГГГГ (N лет)»);
• город/страна;
• грейд/позиция;
• опыт: компании, роли, домены, проекты (подробно);
• стек технологий;
• языки;
• контакты (телефон, Telegram, email);
• проверку полноты оформления (структура проектов).
...
{{
  ...
  "languages": [""],
  "contacts": [""],
  "structure_check": "Полная / Частичная / Отсутствует"
}}

─────────────────────────────
ШАГ 3. СООТВЕТСТВИЕ ТРЕБОВАНИЯМ:
Блок 1 — ОБЯЗАТЕЛЬНЫЕ (must-have)
  • оценка: «Да» / «Нет (требуется уточнение)» / «Нет (точно нет)»

Блок 2 — БУДЕТ ПЛЮСОМ (nice-to-have)
  • оценка: «Да» / «Нет (требуется уточнение)»
...
─────────────────────────────
ШАГ 4. ИТОГ ПО КАНДИДАТУ:
• Сделай общий вывод:  
  - «Полностью подходит» — если все must-have = Да;  
  - «Частично подходит» — если есть хотя бы один «Нет (требуется уточнение)»;  
  - «Не подходит» — если есть хотя бы один «Нет (точно нет)».  
• Добавь зарплатные ожидания (если указаны), иначе «нужно уточнить».  
• В поле `summary` укажи:  
  - «Полностью подходит» — ФИО/ФИ, грейд, локация, ключевой стек, зарплатные ожидания;  
  - «Частично подходит» — ФИО/ФИ и список нужных уточнений;  
  - «Не подходит» — ФИО/ФИ и непройденные must-have.

🧩 JSON:
{{
  "result_status": "Подходит / Нужно уточнение / Отказ",
  "salary_expectations": "",
  "summary": ""
}}
...
─────────────────────────────
📤 ВЫВОД:
Верни строго один JSON в формате:
{{
  "vacancy": {{...}},
  "candidate": {{...}},
  "comparison_tables": {{...}},
...
❗️Не добавляй пояснений, текста, markdown, комментариев или заголовков.  
