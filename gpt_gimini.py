...
def sverka_vac_and_resume_json(vacancy_text: str, resume_list: list[str]):
    ...
    prompt = f"""
PROMPT-1 — ШАГИ 1–5 (Анализ вакансии и всех кандидатов)

1) ВАКАНСИЯ — зафиксировать:
   • позиция и ID;
   • грейд;
   ...
   • стек технологий;
   • особые условия (NDA, SIM +7, ставка, дедлайны и др.);
   • НИК (Telegram-ник менеджера, если указан).

2) КАЖДЫЙ КАНДИДАТ — зафиксировать:
   • ФИО/ФИ (перенести отчество, если есть в оригинале);
   • дата рождения (для WL: «ДД.ММ.ГГГГ (N лет)»);
   • город/страна;
   • грейд/позиция;
   • опыт: компании, роли, домены, ПОДРОБНО РАСПИСАННЫЕ ПРОЕКТЫ;
   • стек технологий;
   • языки;
   • контакты;
   • проверка полноты оформления (включая структуру проектов).

3) ТАБЛИЦЫ СООТВЕТСТВИЯ:
   Блок 1 — ОБЯЗАТЕЛЬНЫЕ
   № | Требование | Статус (Да / Нет (требуется уточнение) / Нет (точно нет)) | Комментарий (подтверждение: годы/уровень/контекст/проекты)
   Блок 2 — БУДЕТ ПЛЮСОМ
   № | Требование | Статус (Да / Нет (требуется уточнение)) | Комментарий

4) ИТОГ ПО КАНДИДАТУ:
   • Полностью подходит / Частично подходит (нужны уточнения) / Не подходит.
   • Зарплатные ожидания — указать, если есть; если нет — отметить «нужно уточнить».

5) СВОДКА ПО ВСЕМ КАНДИДАТАМ:
   • Полностью подходят (финалисты) — ФИО/ФИ, грейд, локация, ключевой стек, зарплатные ожидания.	
   • Частично подходят — ФИО/ФИ и список нужных уточнений.
   • Не подходят — ФИО/ФИ и непройденные must-have.
