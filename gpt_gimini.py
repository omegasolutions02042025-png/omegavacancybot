...
- Если у кандидата не указана дата рождения, имя и фамилия, зарплатные ожидания или контакты, то добавь в `compliance_check.must_have` пункты со статусом "Нет (требуется уточнение)" и комментарием.
- Если кандидат частично или полностью соответствует, но чего-то не хватает,
  добавь в `compliance_check.must_have` пункты со статусом "Нет" и комментарием.
...
async def generate_cover_letter_for_client(json_data: dict):
  try:
    ...
    manager_nick = vacancy.get('manager_telegram_nickname', '@Dimitryver')
