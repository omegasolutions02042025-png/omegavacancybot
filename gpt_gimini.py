...
ЛОГИКА СОВПАДЕНИЙ
— Если любое требование из must_have или nice_to_have отсутствует в данных (невозможно подтвердить по резюме/вакансии), ставь по нему "Нет (требуется уточнение)" (НЕ "Нет (точно нет)").
— Если явно противоречит требованиям (кандидат этого точно не имеет) — "Нет (точно нет)".
— Если в требованиях по кандидату при подаче есть требования к фамилии, имени, отчеству, локации, зарплатным ожиданиям, образованию или опыту работы и их нет в резюме — ставь по такому требованию "Нет (требуется уточнение)".
...
"""
    ...
    model = genai.GenerativeModel("gemini-2.5-pro")
    generation_config = genai.types.GenerationConfig(temperature=0.0, top_p=0.1, top_k=20, candidate_count=1)
...
async def generate_mail_for_candidate_utochnenie(json_data: dict, user_name: str, vacancy_text : str):
  try:
    vacancy = json_data.get('vacancy', {})
    candidate = json_data.get('candidate', {})
...
async def generate_cover_letter_for_client(json_data: dict):
  try:
    ...
    # Оставляем только требования со статусом "Да"
    def only_yes(lst):
      out = []
      for x in lst:
        if (x or {}).get('status') == 'Да':
          out.append({
            ...
            # короткое пояснение (если есть). На выводе оно будет добавлено после маркера.
    ...
    must_yes_json = _json.dumps(must_yes, ensure_ascii=False)
    nice_yes_json = _json.dumps(nice_yes, ensure_ascii=False)

    # Жёсткий шаблон: выводим ТОЛЬКО подтверждённые (Да) требования
    prompt = f"""
...
— Не используй списки и Markdown. Каждый пункт выводи строго в формате двух строк, как указано.
