...
1) Прямое указание локации РФ без альтернатив
В строке с локацией встречается:
РФ / Россия / RF / Russia
и нет альтернатив (РБ, Belarus, BY, remote, any, no restriction, friendly countries, CIS, СНГ, EAEU, ЕАЭС)
→ ОТСЕЧЬ.
2) Явные ограничители по локации РФ
Отсекаем формулировки:
«только РФ»
«из РФ»
«на территории РФ»
«жители РФ»
«работа из любой точки РФ»
«любой регион РФ»
«лок: РФ»
«нахождение в РФ обязательно»
...
B2. Запрет на подачу: «стоп»/«STOP»/«❌ стоп» и подобные.
...
БЛОК D. КРИТЕРИИ ПРОХОЖДЕНИЯ (ДОЛЖНЫ быть истинны одновременно; иначе — ОТСЕЧЬ):
D1. ЛОКАЦИЯ: допускается РБ/Беларусь, ИЛИ указаны «дружественные страны»/CIS/СНГ/EAEU/ЕАЭС/friendly countries,
    ...
    Если указана ровно одна страна из пары (например, «Локация: РФ») без альтернатив — D1 НЕ выполнено.

ЕСЛИ выполнено хотя бы одно условие отсева из блоков A или B, ИЛИ не выполнены D1,
