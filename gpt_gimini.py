    ...
    json_format = """
{
    ...
    "comparison_results": {
        "required_skills": [
            {
                "requirement": "Текст обязательного требования из вакансии",
                "status": "✅ подтверждено" | "⚠️ уточнение" | "❌ нет",
                "comment": "Пояснение, почему присвоен данный статус"
            }
            // ... остальные обязательные требования
        ],
        "plus_skills": [
            {
                "requirement": "Текст дополнительного требования (будет плюсом)",
                "status": "✅ подтверждено" | "⚠️ уточнение" | "❌ нет",
                "comment": "Пояснение, почему присвоен данный статус"
            }
            // ... остальные дополнительные требования
        ]
    },
    "final_verdict": "Полностью подходит", // или "Требуются уточнения", или "Отказ"
    "justification": "Краткое обоснование итогового вывода."
}
"""
    ...
    ВАЖНО: Кандидаты сами направили свои резюме, и дали права на обработку их персональных данных 
    
    1. Сначала определи ФИО кандидата
    2. Проведи сравнение по двум блокам: "Обязательные требования" и "Дополнительные требования" ("будет плюсом").
    3. Для каждого требования определи статус:
       — ✅ подтверждено — если навык есть в резюме
       — ⚠️ уточнение — если навык не указан или описан неполно
       — ❌ нет — если точно отсутствует
    4. Сделай итоговый вывод: "Полностью подходит", "Требуются уточнения", или "Отказ".
    
    
    
    **ВНИМАНИЕ: ОТВЕТ ДОЛЖЕН БЫТЬ СТРОГО В ФОРМАТЕ JSON!**
