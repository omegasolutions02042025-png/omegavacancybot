...
— Блок 2. Дополнительные требования («будет плюсом») — проверять тоже как обязательные. 
 
Для каждого требования укажи: 
✅ подтверждено — если есть в резюме 
⚠️ уточнение — если навык не указан или описан неполно 
❌ нет — если точно отсутствует 
 
Итог: сделай вывод, подходит ли кандидат полностью, или нужны уточнения, или отказ. 
...
def anonymize_contacts(text: str) -> str:
    """
    Очищает текст от персональных контактных данных с помощью набора регулярных выражений.
    
    Удаляет:
    - Email-адреса
    - Телефонные номера (в разных форматах)
    - Ссылки на веб-сайты и соцсети (http/https/www)
    - Имена пользователей (Telegram, Skype и т.д. через @)
    - Ссылки на LinkedIn, GitHub, HeadHunter
    """
    
    # 1. Email-адреса
    # Находит стандартные email-адреса.
    text = re.sub(r'[\w\.\-+=]+@[\w\.\-]+\.[\w]+', '[EMAIL УДАЛЕН]', text)
    
    # 2. Телефонные номера
    # Одно большое выражение, которое покрывает большинство форматов:
    # +7 (999) 123-45-67, 8-999-123-4567, 7 999 123 45 67 и т.д.
    # Оно ищет последовательности цифр, возможно с +, скобками, пробелами и дефисами.
    phone_pattern = r'(?:\+?\d{1,3}[-\s]?)?\(?\d{3}\)?[-\s]?\d{3}[-\s]?\d{2}[-\s]?\d{2}'
    text = re.sub(phone_pattern, '[ТЕЛЕФОН УДАЛЕН]', text)
    
    # 3. Ссылки на веб-сайты и соцсети
    # Находит http/https и www ссылки.
    url_pattern = r'(?:https?://|www\.)[^\s<>"\']+'
    text = re.sub(url_pattern, '[ССЫЛКА УДАЛЕНА]', text)
    
    # 4. Пользовательские имена (Telegram, Skype, etc.)
    # Находит слова, начинающиеся с @, а также префиксы skype: и t.me/
    ...
    # 5. ФИО - Самая сложная и ненадежная часть!
    # Вместо того чтобы искать все слова с большой буквы (что приведет к ошибкам),
    # лучше искать ФИО после ключевых слов, характерных для резюме.
    # (?-i) - временно включает чувствительность к регистру для [А-ЯЁ]
    fio_pattern = r'(?i)(ФИО|Кандидат|Соискатель|Резюме)[:\s]+((?-i)[А-ЯЁ][а-яё]+(\s+[А-ЯЁ][а-яё]+){1,2})'
    text = re.sub(fio_pattern, r'\1: [ФИО УДАЛЕНО]', text)
