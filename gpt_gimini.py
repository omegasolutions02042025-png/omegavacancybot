...
async def generate_mail_for_candidate_utochnenie(json_data: dict):
  ...
  1. Поблагодарить кандидата за отклик.
  2. Коротко и ясно представить вакансию (позиция, стек, формат, условия).
  3. Упомянуть требования, которые нужно уточнить (по данным из анализа).
  4. Использовать деловой, вежливый, но дружелюбный стиль (без излишнего официоза).
  ...
  4️⃣ Сильные стороны кандидата:
    Сравни {vacancy.get('tech_stack')} с {candidate.get('tech_stack')} и укажи 2–4 пункта, по которым кандидат лучше всех.
    Напиши 'Ваш опыт в (результатах сравнения) полностью соответствует требованиям вакансии.'

  5️⃣ Заключение:
  -Возьми список {utochnenie} и кратко напиши что нужно уточнить кандидату для продолжения сверки. Если там написано что нет даты рождения ,имени и фамилии , зарплатных ожиданий то обязательно они должны быть в уточнении.
  -Напиши "Чтобы завершить сверку, пожалуйста, уточните:" и напиши это здесь.
