...
- Находи всегда в тексте резюме имя фамилию и по возможности отчество. Если они указаны на английском, то всегда переводи их на русский язык.
...
- Если у кандидата отсутствуют персональные данные (фамилия, имя, дата рождения, зарплатные ожидания или контакты), 
  добавь для каждого из них отдельный пункт в блок `compliance_check.must_have`.
  Формат для каждого отсутствующего поля:
  {{
  ...
  - **В каждом элементе `compliance_check.must_have` обязательно должны присутствовать все три поля:**
`requirement`, `status`, `comment`.

- Поле `status` **всегда обязательно**, даже если модель не уверена.
Разрешены только эти три значения:
- "Да"
- "Нет (требуется уточнение)"
- "Нет (точно нет)"

❗Если не можешь определить точно — выбери "Нет (требуется уточнение)".
❗Если нет информации вообще — "Нет (точно нет)".

- Поле `comment` также обязательно. Если нечего указать, напиши:
`"comment": "Информация отсутствует в резюме"`

- Если таких пунктов больше 2 — поставь `"summary.verdict": "Частично подходит (нужны уточнения)"`.
- Если почти все требования не подтверждены — `"summary.verdict": "Не подходит"`.
- Если все обязательные соответствуют — `"summary.verdict": "Полностью подходит"`.

- Для каждого требования из `must_have` из вакансии создай отдельный объект:
{{
"requirement": "<текст требования>",
"status": "<Да / Нет (требуется уточнение) / Нет (точно нет)>",
"comment": "<обоснование с примерами из резюме>"
}}


- Для блока `nice_to_have` — аналогично, но статусы только:
- "Да"
- "Нет (требуется уточнение)"

- candidate.location.city и country: всегда переводи на русский язык.
- birth_date.age — рассчитай текущий возраст по дате.
  

- candidate.location.city: Извлеки город из текста [РЕЗЮМЕ]. Если город указан на английском, то всегда переводи его на русский язык.
- candidate.location.country: Извлеки страну из текста [РЕЗЮМЕ]. Если страна указана на английском, то всегда переводи её на русский язык.
- В поле birth_date.age рассчитай возраст кандидата на текущую дату.
- В блоке experience подробно распиши каждый проект, над которым работал кандидат в рамках одной компании. Если проекты не выделены явно, структурируй обязанности как описание одного проекта.
- compliance_check: Это самый важный аналитический блок.
- must_have: Пройдись по каждому обязательному требованию (must_have) из вакансии. Для каждого требования:
- requirement: Скопируй текст требования.
- status: Оцени, есть ли у кандидата соответствующий опыт, и выбери один из трех статусов: "Да", "Нет (требуется уточнение)", "Нет (точно нет)".
- comment: Обоснуй свой статус, ссылаясь на конкретные проекты, компании или навыки из резюме. Укажи релевантный опыт (например, "3 года в компании X", "работал с этим в проекте Y").
- nice_to_have: Сделай то же самое для дополнительных требований (nice_to_have). Статусов здесь два: "Да" или "Нет (требуется уточнение)".
- summary: Сделай итоговый вывод по кандидату.
- verdict: На основе анализа compliance_check.must_have, выбери один из трех вариантов: "Полностью подходит", "Частично подходит (нужны уточнения)", "Не подходит".
- salary_expectations: Укажи зарплатные ожидания, если они есть в резюме. Если нет, напиши "нужно уточнить".
- candidate.full_experience: Укажи полный опыт работы в IT кандидата.
