...
- Если в обязательных требованиях есть требования к ФИО, локации или зарплате то ставь по такому требованию `"Нет(требуется уточнение)"`.
...
## 🧩 ВЕРДИКТ (summary.verdict)
- Если в `must_have` есть хотя бы один пункт `"Нет (точно нет)"`, **кроме** полей:
  — `"ФИО"`, `"Имя"`, `"Фамилия"`, `"Дата рождения"`, `"Возраст"`, `"Локация"`, `"Город"`, `"Контакты"`
  → тогда **"Не подходит"**.
