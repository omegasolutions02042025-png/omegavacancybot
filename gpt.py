...
❌ Если условия оплаты превышают: 35 рабочих дней или 50 календарных дней или содержат запись "акты поквартально" или "поквартальное актирование" или "квартальное актирование" или 'поквартальная оплата' или "актирование", вакансия отсекается.

❌ Если в тексте есть слово 'стоп' или "СТОП" то вакансия отсекается.

Если в тексте есть продолжительность проекта и если она меньше трех месяцев, вакансия отсекается

Проанализируй текст вакансии и найди все упоминания сроков подачи резюме в формате "до DD.MM.YYYY" или "до DD.MM.YYYY HH:MM".  

1. Если дата уже прошла или равна текущей дате, пропусти эту вакансию (не добавляй её в результат).  
2. Если дата еще не наступила, добавь её в результирующий словарь под ключ "deadline_date".  
...
Найди блок контактов в формате:

Вопросы и предложения ➡️@username или в общий чат. Указать 🆔 запроса.


где @username и номер после 🆔 могут быть любыми.
...
Удали из текста все контакты, которые не относятся к этому блоку:

Все упоминания пользователей Telegram (@...) вне этого блока.

Все номера телефонов в любом формате.

Все email-адреса.

Все ссылки (http, https и без протокола).

Не удаляй и не изменяй эмодзи, текст и форматирование остального текста.

Сохрани разметку и абзацы без изменений.


Найди все блоки в тексте, заголовки которых начинаются с любого из следующих вариантов:
...
Любой из вышеуказанных с символом ➕ перед заголовком, например ➕ Будет плюсом, если вы

Действия с найденными блоками:

Захвати весь текст блока до следующего пустого абзаца или нового заголовка.

Удали только заголовок блока (включая символ ➕, если он есть), оставив весь текст.

Объедини все такие тексты в один единый пункт и добавь в раздел «Требования».

Сохраняй все эмодзи, абзацы и форматирование текста внутри блока.

Остальной текст документа оставь без изменений и с сохранением всех абзацев и форматирования.
...
➕ Будет плюсом, если вы
Опыт работы с сетевым оборудованием, знание протоколов TCP/IP. ⚡

Желательно
Знание Linux и навыки администрирования серверов. 🖥️

Обязанности
Настройка и поддержка серверов.


Результат в разделе «Требования»:
...
Остальной текст документа остаётся без изменений, с сохранением всех абзацев:

Обязанности
Настройка и поддержка серверов.

Если в начале текста есть строка, похожая на заголовок вакансии (например: "🥇 Java разработчик c Camunda"), удали её из текста и сохрани в отдельный ключ "vacancy_title".

"vacancy_title" — строка без эмодзи и лишних пробелов, только название вакансии.

Найди в тексте первый встреченный идентификатор вакансии, который выглядит как:

1. 🆔 + пробел + код формата XX-1234 (где XX — латинские буквы в верхнем регистре, а 1234 — цифры)
2. 🆔 + пробел + только цифры (например: 🆔 8581)

Если найден идентификатор:
- Удали его из текста и лишние пробелы вокруг.
- Верни его целиком (буквы + цифры или только цифры) в ключ "vacancy_id", но без символа 🆔.
- Если идентификатора нет — верни "vacancy_id": null.

⚠ В очищенном тексте ("text") обязательно сохраняй все исходные переносы строк и абзацы.
...
В очищенном тексте ("text") обязательно сохраняй все исходные переносы строк, абзацы и форматирование, включая пустые строки.  
Не изменяй порядок строк и не удаляй лишние переносы, кроме тех, что относятся к удалённым частям текста.  
Если в исходном тексте был абзацный отступ или пустая строка между абзацами — оставь их.  

В поле "text" значение должно быть многострочной строкой с сохранением всех переносов строк (\n) как в оригинале.  
